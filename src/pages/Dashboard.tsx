    ...
    <motion.div variants={cardVariants} className="sm:col-span-2 xl:col-span-12">
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          ...
          >
            {t('dashboard.all_courses')} <ChevronRight className="h-3 w-3" />
          </Link>
        </div>
        <div className="space-y-3">
          {courses.map((c) => {
                  ...
                  <div className="relative flex items-center gap-5">
                    ...
                    <div className="flex-1">
                      <div className="mb-0.5 flex items-center gap-2">
                        <h3 className="text-lg font-bold">{t('dashboard.daily_challenge_title')}</h3>
                        <span className="rounded-full bg-white/20 px-2 py-0.5 text-xs font-semibold">{t('dashboard.daily_challenge_count')}</span>
                      </div>
                      <p className="text-sm text-white/70">{t('dashboard.daily_challenge_desc')}</p>
                    </div>
                    <div className="flex flex-col items-end gap-1">
                      {user?.streak ? (
                        <div className="flex items-center gap-1 rounded-full bg-white/20 px-2 py-1 text-xs font-semibold">
                          <Flame className="h-3.5 w-3.5" /> {user.streak}{t('common.days')}
                        </div>
                      ) : null}
                      <ArrowRight className="h-5 w-5 text-white/60 transition-transform group-hover:translate-x-1" />
                    </div>
                ...
                <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 p-6 text-white transition-all hover:shadow-xl hover:shadow-violet-300">
                  ...
                  <div className="relative flex items-center gap-5">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-white/70">{t('dashboard.startup_lab_desc')}</p>
                    </div>
                    <ArrowRight className="h-5 w-5 text-white/60 transition-transform group-hover:translate-x-1" />
                  </div>
                </div>
