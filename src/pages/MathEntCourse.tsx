...
import { MATH_ENT_COURSE } from '@/data/mathEntCourse'
import type { Module, Lesson, QuizQ } from '@/data/mathEntCourse'
import { cn } from '@/lib/utils'

// ── Storage ───────────────────────────────────────────────────────────────────

const STORAGE_KEY = 'studyhub-math-ent-progress'
...
const allLessons = MATH_ENT_COURSE.flatMap(m => m.lessons)
const totalLessons = allLessons.length

function lessonModule(lessonId: string): Module | undefined {
  return MATH_ENT_COURSE.find(m => m.lessons.some(l => l.id === lessonId))
}
...
function Sidebar({ progress, activeId, onSelect }: {
  progress: Progress
  activeId: string | null
  onSelect: (id: string) => void
}) {
  const [openModules, setOpenModules] = useState<Set<string>>(new Set([MATH_ENT_COURSE[0]!.id]))
    ...
    <div className="min-h-screen flex items-center justify-center p-6">
      ...
      >
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-6 border border-blue-200">
          ...
          <p className="text-blue-700 font-bold text-lg mt-2">«Математика — полный курс ЕНТ»</p>
...
export default function MathEntCourse() {
  ...
  const activeMod    = lessonModule(activeLesson.id) ?? MATH_ENT_COURSE[0]!
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="sticky top-0 z-30 bg-white border-b border-slate-200 shadow-sm">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="font-bold text-slate-800 text-sm truncate">📐 Математика ЕНТ — Полный курс</h1>
